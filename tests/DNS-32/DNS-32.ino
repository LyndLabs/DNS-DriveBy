...
// SoftwareSerial gpsSerial(2, 0); // RX, TX
...
void setup() {
    ...
    // Serial.begin(9600);
    Serial2.begin(9600,SERIAL_8N1,16,18);
    Serial.begin(9600);
...
bool makeDNSRequest(char *url) {
    ...
    if (success) {Serial.printf("[+] Pushing (%d/%d): Success!\n\r"),pushCounter.get(),dataCounter.get();}
    else         {Serial.printf("[+] Pushing (%d/%d): Failure :(\n\r"),pushCounter.get(),dataCounter.get();}
